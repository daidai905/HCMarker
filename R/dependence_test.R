...
score1 <- function(obj, tissue, article, markertxt, filename, species) {
  species <- species
  obj_markers <- read.table(markertxt, sep = '\t', header = TRUE)
  raw_count <- obj[["RNA"]]@data
  
 
  score_pre <- data.frame()
  
 
  for (type in unique(obj_markers$cluster)) {
    marker_gene <- obj_markers[obj_markers$cluster == type, "gene"]
    
   
    raw_count_sum <- Matrix::rowSums(raw_count[rownames(raw_count) %in% marker_gene, ],na.rm = TRUE)
    
  
    obj$celltype <- Seurat::Idents(obj)
    sub_count <- obj[["RNA"]]@data[, obj$celltype == type]
    sub_count_sum <- Matrix::rowSums(sub_count[rownames(sub_count) %in% marker_gene, , drop = FALSE])
    ...
    score <- data.frame(
      ...
      Marker = marker_gene,
      Article = article,
      SubSum = sub_count_sum,
      AllSum = raw_count_sum,
      Proportion = sub_count_sum / raw_count_sum
    )
  ...
  write.table(score_pre, filename, sep = "\t", row.names = FALSE, quote = FALSE)
}
...
ct1_sx <- function(obj = obj,tissue = tissue,paper = paper){
  ...
  score1(obj,tissue,paper,"find_marker.txt","ct1_score.txt",species)
